...
def test_slide_metrics_allow_empty_overrides_root(monkeypatch, tmp_path: Path) -> None:
    metrics_root = tmp_path / "registry" / "metrics"
    metrics_root.mkdir(parents=True)
    ...
    pack = PackConfig.model_validate(
        {
            ...
            "context": {
                "metrics": {
                    "allow_empty": False,
                    "bindings": {"instructions_received": "total_instructions"},
                }
            },
            "slides": [
                {
                    ...
                    "context": {
                        "metrics": {
                            "allow_empty": True,
                            "bindings": {"documents_sent": "count_docs_sent"},
                        }
                    },
                    "visual": {"ref": "dummy.yaml"},
                }
    ...
    def fake_resolve_metric_context(
        *,
        ...
        calls.append((scope, allow_empty))
        if scope == "root":
            return ResolvedMetricContext(
                aliases={"total_instructions": 7.0},
                by_key={"instructions_received": 7.0},
